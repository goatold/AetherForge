...
const getProfileDir = (userId: string) =>
  path.join(process.cwd(), ".aetherforge-browser-sessions", "claude-web", userId);
...
const assertLoggedIn = (url: string) => {
  ...
  if (lowered.includes("login") || lowered.includes("signin") || lowered.includes("auth")) {
    throw new Error("Claude web session is not logged in. Open claude.ai and complete login.");
  }
};

export async function runClaudeWebPrompt(userId: string, prompt: string): Promise<string> {
