...
export async function POST() {
  ...
  const conceptsResult = latestArtifactId
  ...
  const payload = await generateQuizPayload(workspace.topic, difficulty, conceptsResult.rows);
  ...
  try {
    ...
    const quizResult = await client.query<{
      ...
      [workspace.id, payload.title, latestArtifactId, payload.provider, payload.model]
    );
